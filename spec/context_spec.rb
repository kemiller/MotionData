module MotionData
  ...
  describe Context do
    ...
    describe "concerning the objects managed by the context" do
      it "returns objects from a different context" do
        authors_in_main_context = [
          Author.new(:name => "Edgar Allan Poe"),
          Author.new(:name => "Stephenie Meyer")
        ]

        context = Context.context
        authors_in_other_context = context.objectsInContext(*authors_in_main_context)
        authors_in_other_context.map(&:objectID).should == authors_in_main_context.map(&:objectID)
        authors_in_other_context.map(&:managedObjectContext).should == [context, context]
